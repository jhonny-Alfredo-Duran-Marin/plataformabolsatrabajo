...
export class ProfesionalComponent implements OnInit {
  ...
  cargarTodo(): void {
    ...
    this.http.get<Formacion[]>(`${this.apiBase}/formacion`, { headers: this.headers() }).subscribe((r) => (this.formaciones = r));
    this.http
      .get<Experiencia[]>(`${this.apiBase}/experiencia`, { headers: this.headers() })
      .subscribe((r) => (this.experiencias = r));
    this.http.get<Idioma[]>(`${this.apiBase}/idiomas`, { headers: this.headers() }).subscribe((r) => (this.idiomas = r));
    this.http
      .get<Certificacion[]>(`${this.apiBase}/certificaciones`, { headers: this.headers() })
      .subscribe((r) => (this.certificaciones = r));
    this.http
      .get<Habilidad[]>(`${this.apiBase}/habilidades`, { headers: this.headers() })
      .subscribe((r) => (this.habilidades = r));
  }

  private refrescarPerfil(): void {
    this.http.get<Perfil>(this.apiBase, { headers: this.headers() }).subscribe((perfil) => (this.perfil = perfil));
  }
    ...
    this.http
      .patch(
        this.apiBase,
        {
          ...
          titulo_profesional: this.perfil.titulo_profesional,
          resumen_profesional: this.perfil.resumen_profesional,
        },
  ...
  eliminarHabilidad(nombre: string): void {
    const nombres = this.habilidades.map((h) => h.nombre).filter((n) => n !== nombre);
    this.http.put<Habilidad[]>(`${this.apiBase}/habilidades`, { habilidades: nombres }, { headers: this.headers() }).subscribe(() => {
      this.cargarTodo();
    });
  }
