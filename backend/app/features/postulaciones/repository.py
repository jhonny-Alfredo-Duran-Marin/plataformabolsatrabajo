...
class PostulacionRepository:
    """Acceso a datos para el seguimiento de postulaciones del egresado (HU-15)."""
    ...
    def listar_postulaciones_candidato(
        ...
        if estado and estado.strip():
            ...
            if estado_clean in ("in_review", "screening"):
        ...
        if fecha_desde:
            stmt = stmt.where(Application.applied_at >= datetime.combine(fecha_desde, time.min))

        if fecha_hasta:
            stmt = stmt.where(Application.applied_at <= datetime.combine(fecha_hasta, time.max))
